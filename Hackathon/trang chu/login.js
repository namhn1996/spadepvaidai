...
loginform.addEventListener("submit", function (e) {
  ...
  for (let i = 0; i < username.length; i++) {
    if (email == username[i].email && password == username[i].password && username[i].status == 0) {
      ...
      // Chuyển trạng thái login về true
      ...
      // ẩn hiện mập mờ
      // Lấy dữ liệu từ local. nếu như chưa có userlogin thì thêm 1 key userlogin có value rỗng
      userlogin = {
    ...
    }else if (email == username[i].email && password == username[i].password && username[i].status == 1){
